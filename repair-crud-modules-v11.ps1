# C:\Users\iesaf\OneDrive\Documentos\TEAC2025-26\Reconstruccion_19102025\inclusive-learning-platform-backend\repair-crud-modules-v11.ps1
<#
repair-crud-modules-v11.ps1
VersiÃ³n: v11
Objetivo: Escanear recursivamente un proyecto multi-mÃ³dulo Maven, detectar entidades Java y generar/repair CRUD (Controller, Service, ServiceImpl, Repository, DTO, Model) para cada entidad.
CaracterÃ­sticas:
 - BÃºsqueda recursiva de todos los .java en la raÃ­z del proyecto.
 - GeneraciÃ³n de estructura (controller, service, service/impl, repository, dto, model).
 - Escritura en UTF-8 sin BOM para todos los archivos (Write-Utf8NoBom).
 - ActualizaciÃ³n/creaciÃ³n de pom.xml en cada mÃ³dulo con propiedades y Spring Boot Maven Plugin que fuerza -Dfile.encoding=UTF8.
 - Reportes CSV + HTML globales en /reports.
 - README.md por mÃ³dulo y README global en la raÃ­z.
 - Compatible con PowerShell 5.1 y 7+ (pwsh).
 - No borrar archivos sin confirmaciÃ³n; reporta acciones.

USO:
 Guardar en la raÃ­z del proyecto inclusive-learning-platform-backend y ejecutar:
   pwsh -File .\repair-crud-modules-v11.ps1
...
# ------------------------------------------------------------
# Encontrar todos los archivos Java recursivamente (no excluir target)
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# Detectar mÃ³dulos (carpetas que contienen src/main/java)
# ------------------------------------------------------------
$moduleRoots = @()
...
if ($moduleRoots.Count -eq 0) {
    Write-Host "[WARN] No se encontraron mÃ³dulos con src/main/java. El script procederÃ¡ escaneando todo el proyecto." -ForegroundColor Yellow
...
# ------------------------------------------------------------
# Procesar cada mÃ³dulo: detectar entidades y generar CRUD
# ------------------------------------------------------------
$moduleSummaries = @()
foreach ($moduleRoot in $moduleRoots) {
    Write-Host "[INFO] Procesando mÃ³dulo: $moduleRoot" -ForegroundColor Cyan
    $moduleJavaRoot = Join-Path $moduleRoot 'src\main\java'
    if (-not (Test-Path $moduleJavaRoot)) { # permitir que tambiÃ©n funcione si .java estÃ¡ en la raiz del mÃ³dulo
        $moduleJavaRoot = $moduleRoot
    }

    # recolectar todas las clases con @Entity en el mÃ³dulo
    $entityFiles = Get-ChildItem -Path $moduleJavaRoot -Recurse -Filter *.java -ErrorAction SilentlyContinue | Where-Object { ($_ | Get-Content -Raw -ErrorAction SilentlyContinue) -match '@Entity' }
    Write-Host "[INFO] Entidades encontradas en mÃ³dulo: $($entityFiles.Count)" -ForegroundColor Green
    ...
    if (-not (Test-Path $pomPath)) {
        ...
        if ($pomText -notmatch '<project.build.sourceEncoding>') {
            $pomText = $pomText -replace '(?s)(<properties>)(.*?)(</properties>)', "\$1\r\n    <project.build.sourceEncoding>UTF-8</project.build.sourceEncoding>\r\n    <project.reporting.outputEncoding>UTF-8</project.reporting.outputEncoding>\r\n\$2\r\n\$3"
            $changed = $true
        }
        if ($pomText -notmatch 'spring-boot-maven-plugin') {
            if ($pomText -match '(?s)</plugins>') {
                $pomText = $pomText -replace '(?s)(</plugins>)', "    <plugin>\r\n      <groupId>org.springframework.boot</groupId>\r\n      <artifactId>spring-boot-maven-plugin</artifactId>\r\n      <configuration>\r\n        <executable>true</executable>\r\n        <jvmArguments>-Dfile.encoding=UTF8</jvmArguments>\r\n      </configuration>\r\n    </plugin>\r\n\$1"
                $changed = $true
            } else {
                # agregar secciÃ³n build/plugins si no existe (caso extremo)
                $insert = "\r\n  <build>\r\n    <plugins>\r\n      <plugin>\r\n        <groupId>org.springframework.boot</groupId>\r\n        <artifactId>spring-boot-maven-plugin</artifactId>\r\n        <configuration>\r\n          <executable>true</executable>\r\n          <jvmArguments>-Dfile.encoding=UTF8</jvmArguments>\r\n        </configuration>\r\n      </plugin>\r\n    </plugins>\r\n  </build>\r\n"
    ...
    # Generar README.md para el mÃ³dulo con resumen de Entidades
    $readmePath = Join-Path $moduleRoot 'README.md'
    $md = "# README - Módulo: $($moduleReport.Module)`r`n`r`n"
    $md += "**Ruta del módulo:** $moduleRoot`r`n`r`n"
...
"
    ...
    $md += "`r`n_Generado por repair-crud-modules-v11.ps1_`r`n"
...
$rootMd += "Generado por repair-crud-modules-v11.ps1`r`n`r`n"
$rootMd += "| Módulo | Entidades procesadas | README |`r`n|---|---:|---|`r`n"
foreach ($m in $moduleSummaries) {
    $modName = $m.Module
    $count = $m.Entities.Count
    $relReadme = Join-Path (Split-Path $m.Module -Parent) 'README.md'
    $rootMd += "| $modName | $count | [README](./$modName/README.md) |`r`n"
}
